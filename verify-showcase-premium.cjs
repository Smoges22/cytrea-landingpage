/* Local warm-premium QA. Requires an existing Playwright installation via NODE_PATH.
...
async function metrics(page, route, width) {
  const data = await page.evaluate(() => {
    ...
    const contrast = [];
    for (const element of document.querySelectorAll('h1,h2,h3,main p,.primary-nav a,.action,.quiet-link,.footer-group-label,.colophon nav a,.footer-attribution,.trademark-note,label,.form-help,.intro,.overline,[role="tab"],[data-store-label],[data-store-detail]')) {
    ...
    for (const element of document.querySelectorAll("input[placeholder],textarea[placeholder]")) {
      ...
      const style = getComputedStyle(element), placeholderStyle = getComputedStyle(element, "::placeholder");
      const foreground = rgb(placeholderStyle.color), background = rgb(style.backgroundColor);
      if (!foreground || !background || foreground.length === 4 && foreground[3] !== 1 || background.length === 4 && background[3] !== 1 || Number(placeholderStyle.opacity) !== 1) continue;
      const a = luminance(foreground), b = luminance(background), ratio = (Math.max(a, b) + 0.05) / (Math.min(a, b) + 0.05);
      contrast.push({ text: `Placeholder: ${element.getAttribute("placeholder").slice(0, 60)}`, class: element.className, foreground: placeholderStyle.color, background: background.slice(0, 3), ratio: Math.round(ratio * 100) / 100, minimum: 4.5 });
    }
...
async function warmSystemChecks(page, route, width) {
  const result = await page.evaluate(expectedFont => {
    const visible = element => !!element.getClientRects().length && getComputedStyle(element).visibility !== "hidden" && !element.closest('[aria-hidden="true"]');
    const family = element => getComputedStyle(element).fontFamily.split(",")[0].replace(/["']/g, "").trim();
    const typography = [...document.querySelectorAll("h1,h2,h3,p,a,button,input,select,textarea,legend,figcaption")].filter(visible).map(element => ({ tag: element.tagName, text: (element.textContent || element.name || "").trim().slice(0, 55), family: family(element) }));
    const footer = document.querySelector(".footer-signoff"), footerBox = footer.getBoundingClientRect(), footerStyle = getComputedStyle(footer);
    const footerItems = [...footer.children].filter(visible).map(element => {
      const box = element.getBoundingClientRect(), style = getComputedStyle(element);
      return { text: element.textContent.trim(), centerX: box.x + box.width / 2, centerY: box.y + box.height / 2, left: box.left, right: box.right, textAlign: style.textAlign };
    });
    const parseColor = value => value.match(/[\d.]+/g)?.map(Number);
    const darkColor = color => color && (color.length < 4 || color[3] === 1) && color.slice(0, 3).reduce((a, b) => a + b, 0) / 3 < 105;
    const android = [...document.querySelectorAll('a[data-store="android"]')].filter(visible).map(anchor => {
      const artwork = anchor.querySelector("img[data-store-artwork]");
      let candidate = artwork?.parentElement, dark;
      while (candidate && anchor.contains(candidate)) {
        const style = getComputedStyle(candidate), colors = [...style.backgroundImage.matchAll(/rgba?\([^)]+\)/g)].map(match => parseColor(match[0]));
        if (darkColor(parseColor(style.backgroundColor)) || colors.length && colors.every(darkColor)) { dark = candidate; break; }
        if (candidate === anchor) break;
        candidate = candidate.parentElement;
      }
      const box = dark?.getBoundingClientRect(), style = dark && getComputedStyle(dark);
      const clippedText = dark ? [...dark.querySelectorAll("[data-store-label],[data-store-detail]")].filter(element => {
        const textBox = element.getBoundingClientRect();
        return textBox.left < box.left - 1 || textBox.right > box.right + 1 || textBox.top < box.top - 1 || textBox.bottom > box.bottom + 1 || element.scrollWidth > element.clientWidth + 1;
      }).map(element => element.textContent.trim()) : [];
      return { footer: !!anchor.closest("footer"), state: anchor.dataset.storeState, label: anchor.querySelector("[data-store-label]")?.textContent.trim(), detail: anchor.querySelector("[data-store-detail]")?.textContent.trim(), darkSurface: !!dark, control: dark && { tag: dark.tagName, class: dark.className, width: box.width, height: box.height, radius: style.borderRadius }, clippedText, entireControlLinked: !!dark && anchor.contains(dark), artworkKind: artwork?.dataset.artworkKind };
    });
    return {
      typographyCount: typography.length, fontMismatches: typography.filter(item => item.family !== expectedFont),
      declaredFonts: [...document.fonts].map(font => ({ family: font.family.replace(/["']/g, ""), status: font.status, display: font.display })),
      fontPreloads: [...document.querySelectorAll('link[rel="preload"][as="font"]')].map(link => link.getAttribute("href")),
      footer: { align: footerStyle.textAlign, justifyContent: footerStyle.justifyContent, centerX: footerBox.x + footerBox.width / 2, items: footerItems }, android,
      redundantDownloadNotices: [...document.querySelectorAll('#download [data-download-copy="android-notice"],#download .download-note')].filter(visible).filter(element => element.textContent.trim()).map(element => element.textContent.trim()),
      visibleFooterTrademark: [...document.querySelectorAll("footer .trademark-note")].filter(visible).map(element => element.textContent.trim())
    };
  }, expectedFont);
  report.warmSystem.push({ route, width, ...result });
  const prefix = `${width}px ${route}`;
  check(!result.fontMismatches.length, `${prefix}: inconsistent chosen font ${JSON.stringify(result.fontMismatches.slice(0, 5))}`);
  check(result.declaredFonts.some(font => font.family === expectedFont && font.status === "loaded" && font.display === "swap"), `${prefix}: chosen self-hosted font not loaded with swap`);
  check(!result.declaredFonts.some(font => font.family !== expectedFont && font.status === "loaded"), `${prefix}: another webfont remains loaded`);
  check(result.fontPreloads.length === 1 && result.fontPreloads[0] === expectedFontAsset, `${prefix}: expected one selected-font preload; found ${result.fontPreloads}`);
  ...
  check(!result.redundantDownloadNotices.length, `${prefix}: redundant download explanation is still visible`);
  if (result.visibleFooterTrademark.length) warnings.push(`${prefix}: visible footer trademark text remains; verify legal necessity and de-emphasized treatment.`);
}
...
async function downloadStates() {
  for (const status of ["early-access", "public"]) {
    ...
    for (const route of pageRoutes) {
      await page.goto(base + route);
      // Store artwork is intentionally lazy-loaded below the fold. Scroll the
      // real controls into view before checking decoded image availability.
      for (const image of await page.locator('[data-store] img[data-store-artwork]:visible').all()) {
        await image.scrollIntoViewIfNeeded();
      ...
      const visualState = await page.locator('a[data-store="android"]').evaluateAll(elements => elements.map(anchor => ({
        imageVisible: !!anchor.querySelector("img[data-store-artwork]")?.getClientRects().length,
        imageLoaded: !!anchor.querySelector("img[data-store-artwork]")?.complete && anchor.querySelector("img[data-store-artwork]").naturalWidth > 0,
        visibleEarlyOnly: [...anchor.querySelectorAll("[data-store-early-only]")].filter(element => element.getClientRects().length).length,
        hiddenEarlyOnly: [...anchor.querySelectorAll("[data-store-early-only]")].filter(element => element.hidden).length,
        earlyOnlyCount: anchor.querySelectorAll("[data-store-early-only]").length,
        target: anchor.target, rel: anchor.rel,
        publicBadgeVisible: [...anchor.querySelectorAll('img[data-artwork-kind="badge"]')].some(image => image.getClientRects().length)
      })));
      for (const item of visualState) {
        check(item.imageVisible && item.imageLoaded && item.target === "_blank" && /noopener/.test(item.rel) && /noreferrer/.test(item.rel), `runtime ${status} ${route}: download artwork/accessibility attributes`);
        if (status === "public") check(item.publicBadgeVisible && item.visibleEarlyOnly === 0 && item.hiddenEarlyOnly === item.earlyOnlyCount, `runtime ${status} ${route}: custom Early Access copy was not replaced by the public badge`);
        else check(!item.publicBadgeVisible, `runtime ${status} ${route}: public badge is visible during Early Access`);
      }
      ...
      report.downloadStates.push({ route, status, runtimeControls: stores.length, staticControls: staticStores.length, visualState, sourceConfigMutated: false });
    }
    await ctx.close();
  }
}
...
async function main() {
  ...
  const sourceFiles = ["experiences.css", "experiences.js", "site-config.js", "experience-content.cjs", "build-experiences.cjs", "sync-site.cjs", "cytrea.css", "cytrea.js", "download-config.js", "sync-downloads.cjs", "sync-forms.cjs", "form-config.js", "forms.js", "vendor-intake.js", expectedFontAsset.slice(1), ...pageRoutes.map(route => route === "/" ? "index.html" : `${route.slice(1)}.html`)];
  report.startingSourceHashes = Object.fromEntries(sourceFiles.map(file => [file, digest(fs.readFileSync(path.join(root, file)))]));
  ...
  for (const width of widths) {
    ...
    for (const route of pageRoutes) {
      ...
      await warmSystemChecks(page, route, width);
  ...
  imageFiles.add(downloads.resolve({ ios: { ...downloads.config.ios }, android: { ...downloads.config.android, status: "public" } }).android.artwork);
  for (const image of imageFiles) {
    const file = decodeURIComponent(image.slice(1));
    let original;
    try { original = execFileSync("git", ["show", `${baseline}:${file}`], { cwd: root, maxBuffer: 64 * 1024 * 1024, stdio: ["ignore","pipe","ignore"] }); }
    catch {
      const source = file.replace("images/web/", "images/current-app/").replace(/\.webp$/, ".png");
      check(source !== file && fs.existsSync(path.join(root,source)), `New image lacks genuine source: ${file}`);
      report.imageContracts.push({file, derivativeOf: source, newOptimizedCopy: true});
      continue;
    }
  ...
  report.fontAssets = [...new Set(report.warmSystem.flatMap(item => item.fontPreloads))].map(url => {
    const file = decodeURIComponent(url.replace(/^\//, "")), data = fs.readFileSync(path.join(root, file));
    return { file, bytes: data.length, sha256: digest(data) };
  });
  if (report.fontAssets.reduce((bytes, font) => bytes + font.bytes, 0) > 150000) warnings.push("The active font preload exceeds 150 KB; review whether the bundle is appropriately scoped.");
  await formContracts();
  await downloadStates();
  await require("./verify-experiences-extra.cjs")({ browser, base, context, check, report, out });
  const reduced = await context({ reducedMotion: "reduce" }), reducedPage = await reduced.newPage();
  for (const route of ["/", "/showcase", "/walkthrough", "/product", "/vendor-partners"]) {
  ...
  await reduced.close();
  report.finalSourceHashes = Object.fromEntries(sourceFiles.map(file => [file, digest(fs.readFileSync(path.join(root, file)))]));
  ...
  const summary = { routes: report.routes.length, renderedPages: report.pages.length, warmSystemChecks: report.warmSystem.length, expectedFont, interactions: report.interactions.length, focusChecks: report.focusChecks.length, links: report.links.length, images: report.images.length, imageContracts: report.imageContracts.length, forms: report.forms.length, downloadStates: report.downloadStates.length, blockedRequests: report.blockedRequests.length, errors, warnings: [...new Set(warnings)] };
